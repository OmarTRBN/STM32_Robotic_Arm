 ...
 * @brief Initialize the TCA9548A multiplexer and AS5600 sensor array

 * @param sensors Pointer to the sensor array structure
...
HAL_StatusTypeDef AS5600_Mux_Init(AS5600_Mux_Array* sensors, I2C_HandleTypeDef* hi2c, uint8_t num_sensors) {
    ...
    for (uint8_t i = 0; i < num_sensors; i++) {
        ...
        // Try reading angle register to check if sensor is responsive
 ...
 * @brief Select a specific channel on the TCA9548A multiplexer
 ...
 * @brief Read the angle from a specific AS5600 sensor

 * @param sensors Pointer to the sensor array structure

 * @param sensor_index Index of the sensor (0 to num_sensors-1)

 * @param angle Pointer to store the angle value (0-4095)
 ...
 * @brief Read angles from all configured AS5600 sensors
